...
export default function Question(props) {
  ...
  async function fetchQuestion() {
    try {
      const response = await axios.get(
        `https://fine-puce-chicken-fez.cyclic.app/question/${props.view.id}`
      );
      const questionsData = response.data;
      const updatedQuestions = await Promise.all(
        questionsData.map(async (question) => {
          const answerResponse = await axios.get(
            `https://fine-puce-chicken-fez.cyclic.app/answer/${question._id}`
          );
  ...
  const handleQuestionSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        "https://fine-puce-chicken-fez.cyclic.app/question",
        {
  ...
  const handleAnswerSubmit = (event) => {
    ...
    axios
      .post("https://fine-puce-chicken-fez.cyclic.app/answer", {
        answer,
        questionId,
        userId,
      })
